...
public class InheritedPropertiesProcessor {
	public void addInheritedProperties(ZenModel model) {
		if (model == null) {
			// can be null when an import declaration is declared, but elements from it are
			// never used
			return;
		}
	...
	private Feature processInheritedProperty(LinkedList<Feature> inheritedPropertiesForName) {
		...
		// no need to copy isRestriction() as it should be defined by the user and means
		// that a property overrides another property
