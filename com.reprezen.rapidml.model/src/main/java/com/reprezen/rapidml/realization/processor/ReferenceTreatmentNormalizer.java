...
public class ReferenceTreatmentNormalizer {
	 ...
	 * <li>If it does not, convert it to a reference embed and add the new reference
	 * embed to the container</li>
	 * </ul>
	 * 
	 */
	...
	private void normalizeUserDefinedReferenceLink(final ReferenceLink referenceLink) {
		...
		if (TagUtils.getTagWithName(referenceLink.getReferenceRealization().getLinkDescriptor(), TagUtils.GENERATED)
			...
			// This is a fix for JSON Schema generator which otherwise inlines the
			// referenceLink
			markWithDefaultRealization(referenceLink);
			NamedLinkDescriptor linkDescriptor = ((ServiceDataResource) referenceLink.getTargetResource())
					.getDefaultLinkDescriptor();
	...
	private void normalizeUserDefinedReferenceEmbed(final ReferenceEmbed embed) {
		...
		if (property != null) {
			if (!isDefailtRealization(embed)) {
				// Need to iterate on a copy of NestedReferenceTreatments because it can be
				// modified during
				// normalization,
	...
	private void removeInvalidTreatments(EList<ReferenceTreatment> treatments) {
		ArrayList<ReferenceTreatment> invalidTreatments = Lists.newArrayList(treatments.stream() //
				.filter(el -> false == el.getReferenceElement() instanceof CollectionReferenceElement) // don't process
																										// collection
																										// resource
																										// links
