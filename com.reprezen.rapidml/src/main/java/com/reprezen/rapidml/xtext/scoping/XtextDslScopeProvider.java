...
public class XtextDslScopeProvider extends AbstractDeclarativeScopeProvider {
    ...
    private ZenModel getLibraryModel(URI modelURI) {
        ...
        // Workspace workspace = new EmfWorkspace(eclipseWorkspace);
        // RestModelLoader loader = new DslRestModelLoader(workspace);
        // return loader.load(modelURI);
        // } else { // workaround to load libraries in headless generation mode
        ...
        try (InputStream stream = Resources
                .getResource(XtextDslScopeProvider.class, "libraries/" + modelURI.lastSegment()).openStream()) {
