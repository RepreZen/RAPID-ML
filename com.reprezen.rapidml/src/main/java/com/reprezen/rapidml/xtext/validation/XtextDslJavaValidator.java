...
public class XtextDslJavaValidator extends AbstractXtextDslJavaValidator {
    ...
    /**
     * The inverse keyword and refProp must be specified on both sides of the inverse pair, and must be consistent such
     * that if DT1.r specifies DT2.r as its inverse, then DT2.r must specify DT1.r as its inverse.
     * 
    ...
    /**
     * The container keyword must be used on any reference that is the inverse of a containing reference. The container
     * keyword must not be used on any reference that is not the inverse of a containing reference.
     */
    @Check
    ...
    /**
     * Checks for properties with duplicates names within the same structure and also in the inheritance hierarchy
     */
    ...
    public void checkTypedMessageUsingThisKeyword(TypedMessage message) {
        if (message.isUseParentTypeReference()) {
            // `this` keyword is not supported when the typed message is message-defined (using type keyword).
            if (message.getDataType() != null) {
                ...
                // Property sets and reference treatments are not allowed when this keyword is used
    ...
    public void checkAutoRealizationFileExistsAndIsValid(ResourceAPI resourceAPI) {
        ...
        for (RealizationModelLocation location : resourceAPI.getAutoRealizations()) {
            ...
            // The fact that we ahve an array of locations is a mistake - only the first is ever used, so we
