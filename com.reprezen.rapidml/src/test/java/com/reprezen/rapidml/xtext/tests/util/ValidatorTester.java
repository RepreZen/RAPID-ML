...
public class ValidatorTester<T extends AbstractDeclarativeValidator> extends AbstractValidatorTester {
	...
	public ValidatorTester(T validator, Injector injector) {
		this(validator, injector.getInstance(EValidatorRegistrar.class),
	...
	public ValidatorTester(T validator, EValidatorRegistrar registrar,
			@Named(Constants.LANGUAGE_NAME) final String languageName) {
	...
	public AssertableDiagnostics diagnose() {
		...
		validatorCalled = false;
		AssertableDiagnostics ad = new AssertableDiagnostics(
				(Diagnostic) validator.setMessageAcceptor(validator).getState().chain);
