...
class ReferenceTreatmentValidatorTests {
	...
	def void testValidModel() {
		...
		validate(referenceTreatment).assertOK
	...
	def void testDuplicateReferenceTreatment_simplePath() {
		...
		validate(referenceTreatment).assertErrorContains(errorMessageFragment);
	}

// TODO: check this test, syntax does not comply with grammar
